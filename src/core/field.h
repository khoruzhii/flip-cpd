...
struct B2 {
    ...
    U64 get(int j) const { return (val >> j) & 1; }
    void set(int j, U64 v) { if (v & 1) val |= (U64(1) << j); else val &= ~(U64(1) << j);}
};
...
struct B3 {
    ...
    U64 get(int j) const {U64 mask = U64(1) << j; return ((lo & mask) ? 1 : 0) | ((hi & mask) ? 2 : 0);}
    void set(int j, U64 val) {
        U64 mask = U64(1) << j;
        val = val % 3;
        if (val & 1) lo |= mask; else lo &= ~mask;
        if (val & 2) hi |= mask; else hi &= ~mask;
    }
};
...
// Scalar multiplication for B3
inline B3 operator*(const B3& x, U64 c) {
    c = c % 3;
    if (c == 0) return B3{0, 0};
    if (c == 1) return x;
    return -x;  // c == 2
}

// Compound assignment for convenience
inline B3& operator+=(B3& a, const B3& b) {
    a = a + b;
    return a;
}
